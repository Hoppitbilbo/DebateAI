...
// Define the props for InquiryDialogueChat, including the new handlers
interface InquiryDialogueChatProps {
...
const InquiryDialogueChat = ({ characters, theme, onMessagesUpdate, handleEndChat }: InquiryDialogueChatProps) => {
  const { t } = useTranslation();
  const [messages, setMessages] = useState<Message[]>(() => [
    {
      role: "system",
      character: "System",
      content: t('apps.inquiryDialogue.chat.systemMessage', { char1: characters[0].name, char2: characters[1].name, theme }),
    },
  ...
  const handleUserInput = async (userMessage: string) => {
    ...
    try {
      ...
      addMessage({
        role: "system",
        character: "System",
        content: t('apps.inquiryDialogue.chat.errorMessage')
      });
    }
    setIsLoading(false);
  };
  ...
  const handleContinueDialogue = async () => {
    ...
    try {
      ...
      addMessage({
        role: "system",
        character: "System",
        content: t('apps.inquiryDialogue.chat.autoErrorMessage')
      });
    }
    setIsLoading(false);
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <Tabs defaultValue="dialogue" className="flex flex-col h-full">
          <div className="bg-education p-2 text-white flex justify-between items-center">
            <TabsList className="grid grid-cols-2 w-full bg-gray-700">
              <TabsTrigger value="dialogue" className="data-[state=active]:bg-education-dark data-[state=active]:text-white">{t('apps.inquiryDialogue.chat.dialogueTab')}</TabsTrigger>
              <TabsTrigger value="characters" className="data-[state=active]:bg-education-dark data-[state=active]:text-white">{t('apps.inquiryDialogue.chat.charactersTab')}</TabsTrigger>
            </TabsList>
          </div>
      ...
      >
        <BookOpen className="mr-2 h-4 w-4" /> {t('apps.inquiryDialogue.chat.endDialogueButton')}
      </Button>
    </div>
  );
};

export default InquiryDialogueChat;