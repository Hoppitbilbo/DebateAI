...
import InquiryDialogueReflection from "@/components/InquiryDialogue/InquiryDialogueReflection";
import InquiryDialogueFeedback from "@/components/InquiryDialogue/InquiryDialogueFeedback";
// Import local Message type from inquiry-dialogue for the handler
import { Message as InquiryDialogueMessage } from "@/components/inquiry-dialogue/types";
...
//This interface is used within InquiryDialoguePage and passed to InquiryDialogueChat
...
const InquiryDialoguePage = () => {
  ...
  const fetchWikiSummary = async (title: string, index: number) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('apps.inquiryDialogue.errors.fetchError', { title }));
      const data = await response.json();
      const pages = data.query.pages;
      const pageId = Object.keys(pages)[0];
      const extract = pages[pageId].extract || t('apps.inquiryDialogue.errors.noSummary');
      setCharacterSummaries(prevSummaries => {
        const newSummaries = { ...prevSummaries };
      ...
      setCharacterSummaries(prevSummaries => {
        const newSummaries = { ...prevSummaries };
        newSummaries[index] = t('apps.inquiryDialogue.errors.summaryError');
        return newSummaries;
      });
    }
  };
  ...
  const handleStartDialogue = () => {
    const errorMessage = t('apps.inquiryDialogue.errors.summaryError');
  ...
  const handleMessagesUpdate = useCallback((updatedMessagesFromChat: InquiryDialogueMessage[]) => {
    // Map messages from InquiryDialogueChat's local type to the global Message type
  ...
  const evaluateCurrentReflection = useCallback(async (reflection: string) => {
    if (!selectedCharacters[0] || !selectedCharacters[1] || !theme || messages.length === 0) {
      setAiEvaluation({
        textualFeedback: t('apps.inquiryDialogue.errors.insufficientData', {
          defaultValue:
    ...
    try {
      ...
      setAiEvaluation({
        textualFeedback: t('apps.inquiryDialogue.errors.technicalReflection', {
          defaultValue:
      ...
      return [
        {
          ...
          dialogueStyle: t('apps.inquiryDialogue.characterStyles.erudite')
        },
        {
          ...
          dialogueStyle: t('apps.inquiryDialogue.characterStyles.reflective')
        },
      ];
    }
    return null;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-extrabold sm:text-5xl md:text-6xl text-education-light">
              {t('apps.inquiryDialogue.title')}
            </h1>
            <p className="mt-4 text-xl text-gray-300 max-w-2xl mx-auto">
              {t('apps.inquiryDialogue.description')}
            </p>
            <div className="mt-6">
              <Link to="/apps/inquiry-dialogue/tutorial">
                <Button
                  variant="outline"
          ...
          {currentPhase === "selecting" && (
            <div className="max-w-3xl mx-auto">
              <Card className="bg-gray-800 border-gray-700 shadow-xl">
                <CardHeader>
                  <CardTitle className="text-3xl text-education-light">{t('apps.inquiryDialogue.setup.buildDialogue')}</CardTitle>
                  <CardDescription className="text-gray-400">
                    {t('apps.inquiryDialogue.setup.selectDescription')}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  {[0, 1].map(index => (
                    <div key={index} className="space-y-3 p-4 bg-gray-700/50 rounded-lg">
                      <label className="block text-lg font-medium text-education-lighter">
                        {t(`apps.inquiryDialogue.setup.character${index + 1}`)}
                      </label>
                      ...
                      {selectedCharacters[index] && (
                        <div className="mt-3 p-3 bg-gray-800 rounded-md text-sm text-gray-300 border border-gray-600">
                          ...
                          {characterSummaries[index] === null && <p className="text-xs text-yellow-400 mt-1">{t('apps.inquiryDialogue.setup.loadingSummary')}</p>}
                          {characterSummaries[index] === t('apps.inquiryDialogue.errors.summaryError') && <p className="text-xs text-red-400 mt-1">{characterSummaries[index]}</p>}
                          {characterSummaries[index] && characterSummaries[index] !== null && characterSummaries[index] !== t('apps.inquiryDialogue.errors.summaryError') && (
                            <p className="text-xs text-green-400 mt-1">{t('apps.inquiryDialogue.setup.summaryLoaded')}</p>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="space-y-3 p-4 bg-gray-700/50 rounded-lg">
                    <label className="block text-lg font-medium text-education-lighter">
                      {t('apps.inquiryDialogue.setup.themeLabel')}
                    </label>
                    <Input 
                        placeholder={t('apps.inquiryDialogue.setup.themePlaceholder')}
                        value={theme} 
                  ...
                  >
                    {t('apps.inquiryDialogue.setup.startButton')} <ArrowRight className="ml-2 h-5 w-5" />
                  </Button>
                </CardContent>
              </Card>
            </div>
          ...
          {currentPhase === "reflecting" && (
            <InquiryDialogueReflection onSubmit={handleSubmitReflection} />
          )}

          {currentPhase === "reflecting" && isEvaluating && (
            <div className="max-w-2xl mx-auto text-center py-10">
              <p className="text-2xl text-gray-300">
                {t('apps.inquiryDialogue.status.evaluating', {
...
export default InquiryDialoguePage;